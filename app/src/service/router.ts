...
const classesWithSubscribers = (
  ...
  for (let c of classData) {
    aggregated.push(
      classWithSubscribers(
        c,
        attendees.filter((a) => a.class_id == c._id),
      ),
    );
  }
  return aggregated;
...
router.post('/api/login', (req: Request, res: Response) => {
  ...
  (notEmpty(email) && notEmpty(password)) ||
    logAndSend('missing data', res, 'missing data');
  usersModel
    .getByCondition({ email })
...
router.get('/api/class/:id', (req: Request, res: Response) => {
  ...
  Promise.all([
    ...
    attendeesModel.getByCondition({ class_id: id }),
  ])
    .then(([classData, attendees]) =>
      res.send(classWithSubscribers(classData, attendees)),
    )
...
router.get('/api/class', (req: Request, res: Response) => {
  ...
  Promise.all([classesModel.get(), attendeesModel.get()])
    .then(([classData, attendees]) =>
      res.send(classesWithSubscribers(classData, attendees)),
    )
    .catch((error) => logAndSend(error, res, 'could not retrieve class data'));
});
...
router.post('/api/subscribe', (req: Request, res: Response) => {
  ...
  validateToken(username, req)
    ...
    .then((userData) =>
      attendeesModel.post({ username, class_id, name: userByName(userData) }),
    )
    .then(() => classesModel.getOne(class_id))
    .then((data) => res.send(data))
...
router.get('/api/testing/users', (req: Request, res: Response) => {
  const usersModel = model(users);
  usersModel.get().then((data) => res.send(data));
});
