...
const makeToken = ({ _id: user_id, email }: SavedUser) =>
  jwt.sign({ user_id, email }, process.env.SECURITY_KEY || 'secret', {
    expiresIn: '8h',
  });
...
export const validateToken = (email: string, req: Request) =>
  new Promise(() => {
    const token = req.headers.authorization;
    const { email: tokenEmail, user_id } = jwt.decode(token) as JwtPayload;
    if (email == tokenEmail) {
      throw Error(
        `Authentication error, token: ${tokenEmail}, email: ${email}`,
      );
    }
  });

export const login = (user: SavedUser, password: string) =>
  new Promise((resolve, reject) =>
    bcrypt.compare(password, user.password, (error, result) =>
      error || !result
        ? reject({ error, result, password, hash: user.password })
